...
def simulate_sand(rock_positions: set) -> int:
    depth = max([i[1] for i in rock_positions])
    sands = 0
    sand_x, sand_y = 500, 0
    while sand_y < depth:
...
simulate_sand(copy.deepcopy(rock_pos))
...
def simulate_sand(rock_positions: set) -> int:
    sands = 0
    sand_x, sand_y = 500, 0
    while (sand_x, sand_y) not in rock_positions:
        if (sand_x, sand_y + 1) not in rock_positions:
            sand_y += 1
        elif (sand_x - 1, sand_y + 1) not in rock_positions:
            sand_x -= 1
