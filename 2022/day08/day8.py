...
def calculate_visibility(tree_row: Iterable) -> list[bool]:
    return [
        (i == 0)
        or (i == len(tree_row) - 1)
...
sum([i or j for i, j in zip(horizontal_rows, vertical_rows)])

## Part 2
def calculate_distance(tree_row: Iterable) -> list[bool]:
    return [
...
max([i * j for i, j in zip(horizontal_rows, vertical_rows)])
